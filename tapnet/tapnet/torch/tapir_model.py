...
def _map_structure(fn, value, other=None):
  if isinstance(value, tuple):
    if other is None:
      return tuple(_map_structure(fn, v) for v in value)
    return tuple(_map_structure(fn, v, o) for v, o in zip(value, other))
  if isinstance(value, list):
    if other is None:
      return [_map_structure(fn, v) for v in value]
    return [_map_structure(fn, v, o) for v, o in zip(value, other)]
  if isinstance(value, dict):
    if other is None:
      return {k: _map_structure(fn, v) for k, v in value.items()}
    return {k: _map_structure(fn, v, other[k]) for k, v in value.items()}
  if other is None:
    return fn(value)
