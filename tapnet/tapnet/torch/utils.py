...
def einshape(
  ...
  """Small local subset of einshape used by torch TAPIR."""
  if not isinstance(value, torch.Tensor):
    value = torch.tensor(value)
  if equation == 'bnfc->(bn)fc':
    b, n, f, c = value.shape
    return value.reshape(b * n, f, c)
  if equation == 'bn...->(bn)...':
    b, n = value.shape[:2]
    return value.reshape((b * n,) + tuple(value.shape[2:]))
  if equation == '(bn)fc->bnfc':
    b = int(index_sizes['b'])
    bn, f, c = value.shape
    if bn % b != 0:
      raise ValueError('Cannot infer n for (bn)fc->bnfc')
    return value.reshape(b, bn // b, f, c)
  if equation == '(bn)...->bn...':
    b = int(index_sizes['b'])
    bn = value.shape[0]
    if bn % b != 0:
      raise ValueError('Cannot infer n for (bn)...->bn...')
    return value.reshape((b, bn // b) + tuple(value.shape[1:]))
  if equation == 'tbnhw->(tbn)hw1':
    t, b, n, h, w = value.shape
    return value.reshape(t * b * n, h, w, 1)
  if equation == '(tb)hw1->t(b)hw1':
    t = int(index_sizes['t'])
    tb, h, w, one = value.shape
    if tb % t != 0:
      raise ValueError('Cannot infer b for (tb)hw1->t(b)hw1')
    return value.reshape(t, tb // t, h, w, one)
  if equation == 't(bn)hw1->bnthw':
    b = int(index_sizes['b'])
    n = int(index_sizes['n'])
    t, bn, h, w, _ = value.shape
    if bn != b * n:
      raise ValueError('Provided b/n do not match t(bn)hw1 shape')
    return value.reshape(t, b, n, h, w).permute(1, 2, 0, 3, 4)
  if equation == '(tbn)1->bnt':
    n = int(index_sizes['n'])
    t = int(index_sizes['t'])
    tbn = value.shape[0]
    if tbn % (t * n) != 0:
      raise ValueError('Cannot infer b for (tbn)1->bnt')
    b = tbn // (t * n)
    return value.reshape(t, b, n).permute(1, 2, 0)
  raise NotImplementedError(f'Unsupported local einshape equation: {equation}')
